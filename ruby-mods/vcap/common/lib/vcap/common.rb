...
module VCAP
  ...
  WINDOWS = Config::CONFIG['host_os'] =~ /mswin|mingw/ # TODO T3CF should probably go into common lib
  ...
  def self.secure_uuid
    ...
    # TODO T3CF result = File.open('/dev/urandom') { |x| x.read(16).unpack('H*')[0] }
  end
